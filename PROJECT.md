...
- Prefer atomic, composable tools over prompt-specific backend workflow scripts. A versioned skill may define a durable checkpoint graph when the methodology itself is part of correctness; the coordinator instantiates that graph from structured inputs while adapters retain validation, authorization, and execution control.
...
5. In progress: versioned workflow skills now declare triggers, structured inputs, required tools, checkpoint graphs, completion audits, and deliverables. The scholarly-report workflow is the first migrated skill; import literature-backed Grid study playbooks through the same schema next.
