...
class ToolCoordinator:
    ...
    @staticmethod
    def _narrowed_specs(tools: list[DiscoveredTool], data: dict[str, Any]) -> list[dict[str, Any]]:
        ...
        return [tool.openai_spec() for tool in tools]
    ...
    async def respond(
        ...
        if not tools:
            raise RuntimeError("The selected MCP plugins exposed no tools.")
        ...
        catalog = "\n".join(f"- {tool.exposed_name}: {tool.description[:180]}" for tool in tools)
        plan_prompt = (
        ...
        while call_count < max_calls:
            ...
            specs = self._narrowed_specs(tools, data)
